...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        m_driverController.povUp().whileTrue(ElevatorCommandFactory.setVoltage(m_elevator, 1)).onFalse(
            ElevatorCommandFactory.setVoltage(m_elevator, 0.0)
        );
        m_driverController.povDown().whileTrue(ElevatorCommandFactory.setVoltage(m_elevator, -1)).onFalse(
            ElevatorCommandFactory.setVoltage(m_elevator, 0.0)
        );

        //manual elevator binded to right and left trigger
        m_driverController.rightTrigger().whileTrue(ElevatorCommandFactory.setVoltage(m_elevator, 6)).onFalse(
            ElevatorCommandFactory.setVoltage(m_elevator, 0.0)
        );

        m_driverController.leftTrigger().whileTrue(ElevatorCommandFactory.setVoltage(m_elevator, -6)).onFalse(
            ElevatorCommandFactory.setVoltage(m_elevator, 0.0)
        );
